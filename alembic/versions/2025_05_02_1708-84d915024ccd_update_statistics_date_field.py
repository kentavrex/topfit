...
def upgrade() -> None:
    ...
    op.alter_column(
        "statistics", "created_at", existing_type=sa.DATE(), type_=sa.TIMESTAMP(timezone=True), existing_nullable=False
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "statistics", "created_at", existing_type=sa.TIMESTAMP(timezone=True), type_=sa.DATE(), existing_nullable=False
    )
