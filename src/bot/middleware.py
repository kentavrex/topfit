...
class SaveUserMiddleware(BaseMiddleware):
    ...
    def _get_new_user_intro_message() -> str:
        return (
            f"Что умеет бот TopFit:\n"
            f"- 🎯Поможет определиться с целью по вашей дневной норме КБЖУ (Цель)\n"
            f"- 🧮Посчитает за вас калории блюда по тексту, фото или голосовому сообщению (Добавить блюдо)\n"
            f"- 🔥Будет отслеживать за вас статистику добавленных блюд за день (Статистика)\n"
            f"- 🍽Порекомендует блюдо на основе ваших предпочтений и дневной КБЖУ из цели (AI рекомендация)\n"
        )
